...
def u_plot_fixed_k(df,k = 0,klabel = None, mark_point = None, save = None):
    ...
    The function generates two subplots: one for the real part of u and another for the imaginary part of u.
    
    Args:
        df (pd.DataFrame): DataFrame containing the values of k, b, u_real, and u_complex.
        k (float): Fixed value of k.
        klabel (str): Label for the fixed value of k.
        mark_point (tuple): Coordinates of a point to be marked on the plot.
        save (str): Directory to save the plot.
    returns:
        None
    """
    assert k in df['k'].values , f'k = {k} not in df.k values'
    try:
        if klabel is None:
            klabel = str(k)
        ...
        fig.suptitle(f'$u$-Bifurcation Diagram for $\\hat{{k}} = {klabel}$', fontsize=16)
...
def u_plot_fixed_b(df,b = 0,blabel = None, mark_point = None , save = None):
    """
    ...
    Args:
        ...
        blabel (str): Label for the fixed value of b.
        mark_point (tuple): Coordinates of a point to be marked on the plot.
        save (str): Directory to save the plot.
        ...
        returns:
            None
    """
    assert b in df['b'].values, f'b = {b} not in df.b values'
    try:
        if blabel is None:
            blabel = str(b)
        ...
        fig.suptitle(f'$u$-Bifurcation Diagram for $\\hat{{\\beta}} = {blabel}$', fontsize=16)
...
def u_plot_kb_3d(df, id=None, mark_point_real=None, mark_point_complex=None, save=None, view_angle=(30, -60)):
    try:
        if id is None:
            id = "def"
        ...
        fig.suptitle(f'Bifurcation Diagram for $u$ values', fontsize=16)
